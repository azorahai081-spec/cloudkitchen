 ...
 * PizzaMania: Cloud Kitchen Order Details
 * Version 4.5 - (UPDATED) Moved Desktop Action Button to Header
 */
...
// Surcharge Calc
$base = (int) ($order['base_charge'] ?? 0);
$saved_fee = (int) $order['delivery_fee'];
$adj = (int) ($order['delivery_adjustment'] ?? 0);
...
<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
    ...
    <div>
        <div class="flex items-center gap-3">
            ...
            <span
                class="px-3 py-1 rounded-full text-xs font-bold border uppercase tracking-wider <?php echo $status_badge_class; ?>">
    ...
    <!-- Right: Actions (Desktop) & Override -->
    <div class="flex flex-col sm:flex-row gap-3 items-end sm:items-center">

        <!-- (NEW) Primary Action Button (Hidden on Mobile, Visible on Desktop) -->
        <?php if (!empty($next_action)): ?>
            <form method="POST" class="hidden md:block">
                ...
                <button type="submit" name="update_status"
                    class="<?php echo $btn_color; ?> text-white font-bold py-2 px-5 rounded-lg shadow-md hover:shadow-lg transition transform flex items-center gap-2 text-sm border border-transparent">
        ...
        <form method="POST" class="flex gap-2 items-center">
            <span class="text-xs font-medium text-gray-400 uppercase hidden sm:inline">Override:</span>
            <input type="hidden" name="csrf_token" value="<?php echo get_csrf_token(); ?>">
            <select name="new_status"
                class="text-sm border-gray-300 rounded-lg shadow-sm focus:border-gray-500 focus:ring-gray-500 py-2 bg-white">
                <option value="Pending" <?php echo ($order['order_status'] == 'Pending') ? 'selected' : ''; ?>>Pending
                </option>
                <option value="Preparing" <?php echo ($order['order_status'] == 'Preparing') ? 'selected' : ''; ?>>Preparing
                </option>
                <option value="Ready" <?php echo ($order['order_status'] == 'Ready') ? 'selected' : ''; ?>>Ready</option>
                <option value="Delivered" <?php echo ($order['order_status'] == 'Delivered') ? 'selected' : ''; ?>>Delivered
                </option>
                <option value="Cancelled" <?php echo ($order['order_status'] == 'Cancelled') ? 'selected' : ''; ?>>Cancelled
                </option>
            </select>
            <button type="submit" name="update_status"
                class="bg-white hover:bg-gray-50 text-gray-700 px-3 py-2 rounded-lg text-sm font-medium border border-gray-300 shadow-sm transition-colors">Update</button>
        </form>
    </div>
</div>
...
<?php if ($order['order_status'] != 'Cancelled'): ?>
    <div class="mb-10 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div class="relative flex items-center justify-between w-full max-w-4xl mx-auto">
            ...
            $progress_width = 0;
            if ($current_step > 1)
                $progress_width = ($current_step - 1) * 33.33;
            ...
            ?>
            <div class="absolute left-0 top-[20px] h-1.5 <?php echo $line_color; ?> rounded-full -z-10 transition-all duration-700 ease-in-out"
            ...
            foreach ($steps as $step_num => $data):
                ...
                <div class="flex flex-col items-center z-10">
                    <div
                        class="w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 <?php echo $circle_class; ?>">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="<?php echo $data['icon']; ?>"></path>
                        </svg>
                    </div>
                    <span
                        class="text-xs mt-2 uppercase tracking-wide <?php echo $label_class; ?>"><?php echo $data['label']; ?></span>
                </div>
            <?php endforeach; ?>
        </div>
    </div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <h2 class="font-bold text-gray-800 flex items-center gap-2">
                    ...
                    Items Ordered <span
                        class="bg-gray-200 text-gray-600 text-xs py-0.5 px-2 rounded-full ml-1"><?php echo count($items); ?></span>
                </h2>
                <?php if ($current_step < 4): ?>
                    <a href="edit_order.php?id=<?php echo $order_id; ?>"
                        class="text-sm font-medium text-blue-600 hover:text-blue-800 hover:underline">Edit Order</a>
                <?php endif; ?>
            </div>
            ...
            <div class="divide-y divide-gray-50">
                <?php foreach ($items as $item):
                    ...
                    <div class="p-5 hover:bg-gray-50 transition-colors flex items-start gap-5">
                        <div
                            class="w-16 h-16 rounded-xl bg-gray-100 flex-shrink-0 overflow-hidden border border-gray-200 shadow-sm">
                            <img src="<?php echo e($img_src); ?>" class="w-full h-full object-cover" alt="Item">
                        </div>
                        <div class="flex-1 min-w-0">
                            <div class="flex justify-between items-start">
                                <h3 class="font-bold text-gray-900 text-lg truncate pr-4">
                                    <?php echo e($item['real_name']); ?></h3>
                                <span
                                    class="font-bold text-gray-900 whitespace-nowrap"><?php echo number_format($item['total_price'], 0); ?></span>
                            </div>
                            <div class="mt-1.5 flex flex-wrap gap-2">
                                <?php if (!empty($item['opts'])): ?>
                                    <?php foreach ($item['opts'] as $opt): ?>
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium bg-orange-50 text-orange-700 border border-orange-100">
                                            <?php echo e($opt['option_name']); ?>
                                            (+<?php echo number_format($opt['option_price'], 0); ?>)
                                        </span>
                                    <?php endforeach; ?>
                                <?php else: ?>
                        ...
                        <div
                            class="text-lg font-bold text-gray-400 ml-2 bg-gray-100 px-2 py-1 rounded-lg border border-gray-200">
            ...
            <div class="bg-gray-50/80 p-6 border-t border-gray-200">
                <div class="space-y-3 text-sm text-gray-600 max-w-sm ml-auto">
                    <div class="flex justify-between"><span>Subtotal</span><span
                            class="font-medium text-gray-900"><?php echo number_format($order['subtotal'], 0); ?></span>
                    </div>

                    <?php if ($order['discount_amount'] > 0): ?>
                        <div class="flex justify-between text-emerald-600 bg-emerald-50 px-2 py-1 rounded">
                            <span>Discount</span>
                            <span>-<?php echo number_format($order['discount_amount'], 0); ?></span>
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="flex justify-between">
                        ...
                        <span
                            class="font-medium text-gray-900"><?php echo number_format($order['delivery_fee'], 0); ?></span>
                    </div>
                    ...
                    <?php if ($surcharge > 0): ?>
                        <div class="flex justify-between text-orange-600 bg-orange-50 px-2 py-1 rounded text-xs">
                            ...
                            <span>+<?php echo number_format($surcharge, 0); ?></span>
                        </div>
                    <?php endif; ?>
                    ...
                    <?php if ($adj != 0): ?>
                        <div class="flex justify-between text-gray-500 italic">
                            ...
                            <span><?php echo ($adj > 0 ? '+' : '') . number_format($adj, 0); ?></span>
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="mt-6 pt-4 border-t border-gray-200 flex justify-between items-center">
                    ...
                    <div class="font-extrabold text-3xl text-emerald-600 flex items-baseline gap-1">
                        <?php echo number_format($order['total_amount'], 0); ?> <span
                            class="text-sm font-bold text-emerald-600/60">BDT</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            <h3 class="text-xs font-bold text-black uppercase tracking-wider mb-6 border-b border-gray-100 pb-2">
                Customer Information</h3>

            <div class="flex items-center gap-4 mb-8">
                <div
                    class="w-12 h-12 rounded-full bg-slate-800 text-white flex items-center justify-center font-bold text-xl shadow-md">
                    <?php echo strtoupper(substr($order['customer_name'], 0, 1)); ?>
                </div>
                <div>
                    <p class="font-bold text-gray-900 text-lg leading-tight"><?php echo e($order['customer_name']); ?>
                    </p>
            ...
            <div class="grid grid-cols-2 gap-3 mb-8">
                <a href="tel:<?php echo e($order['customer_phone']); ?>"
                    class="flex items-center justify-center gap-2 py-2.5 px-4 border border-gray-200 rounded-xl text-sm font-semibold text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition shadow-sm">
                ...
                <a href="https://wa.me/<?php echo $wa_phone; ?>" target="_blank"
                    class="flex items-center justify-center gap-2 py-2.5 px-4 bg-[#25D366]/10 border border-[#25D366]/20 rounded-xl text-sm font-semibold text-[#128C7E] hover:bg-[#25D366]/20 transition shadow-sm">
                ...
                <div>
                    ...
                    <form method="POST" class="flex gap-2">
                        ...
                        <div class="relative w-full">
                            <select name="rider_name"
                                class="appearance-none block w-full text-sm border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2.5 bg-white">
                        ...
                        <button type="submit" name="assign_rider"
                            class="bg-blue-600 text-white px-4 rounded-lg text-sm font-medium hover:bg-blue-700 shadow-sm transition-colors">Save</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <a href="print_receipt.php?id=<?php echo $order_id; ?>&copy=chef" target="_blank"
                class="group flex flex-col items-center justify-center p-4 bg-white border border-gray-200 rounded-xl shadow-sm hover:bg-gray-50 hover:border-gray-300 transition-all hover:-translate-y-0.5">
                <div
                    class="w-8 h-8 rounded-full bg-gray-100 text-gray-500 flex items-center justify-center mb-2 group-hover:bg-white group-hover:text-gray-800 transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <span class="text-xs font-bold text-gray-600 uppercase tracking-wide group-hover:text-gray-800">Chef
                    Copy</span>
            </a>
            <a href="print_receipt.php?id=<?php echo $order_id; ?>&copy=customer" target="_blank"
                class="group flex flex-col items-center justify-center p-4 bg-white border border-gray-200 rounded-xl shadow-sm hover:bg-gray-50 hover:border-gray-300 transition-all hover:-translate-y-0.5">
                <div
                    class="w-8 h-8 rounded-full bg-gray-100 text-gray-500 flex items-center justify-center mb-2 group-hover:bg-white group-hover:text-gray-800 transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <span
                    class="text-xs font-bold text-gray-600 uppercase tracking-wide group-hover:text-gray-800">Receipt</span>
            </a>
        </div>
    </div>
</div>
...
<?php if (!empty($next_action)): ?>
    <div class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 shadow-lg z-40 md:hidden">
        <form method="POST" class="grid grid-cols-1 gap-3">
            ...
            <input type="hidden" name="new_status" value="<?php echo $next_status; ?>">
            <button type="submit" name="update_status"
                class="w-full <?php echo $btn_color; ?> text-white font-bold py-3.5 px-4 rounded-xl shadow-md transition flex items-center justify-center gap-2 text-lg">
                <?php echo $next_action; ?>
            </button>
        </form>
    </div>
