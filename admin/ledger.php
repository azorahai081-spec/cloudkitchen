...
/*
 * admin/ledger.php
 * PizzaMania: Cloud Kitchen Admin's Personal Ledger
 * Version 2.2 - (MODIFIED) Manager View: Cost Only
 *
...
// 2. SECURITY CHECK - REMOVED STRICT CHECK
// (MODIFIED) We now allow managers to access this page, but with limited view.
// if (!hasAdminAccess()) { ... }
...
$type_form = 'deposit'; // Default for admins
$amount_form = '';
$description_form = '';
...
if ($action === 'delete' && $entry_id) {
    if (!validate_csrf_token()) {
        ...
        $error_message = 'Access Denied. Only Admins can delete entries.';
    } else {
...
// 5. --- HANDLE EDIT ACTION (ADMIN ONLY) ---
if ($action === 'edit' && $entry_id) {
    if (!hasAdminAccess()) {
        $error_message = 'Access Denied. Only Admins can edit entries.';
        $form_mode = 'add';
    } else {
        $form_mode = 'edit';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_entry'])) {
    if (!validate_csrf_token()) {
        ...
        // (MODIFIED) Force type for non-admins
        if (!hasAdminAccess()) {
            $type = 'withdrawal';
        } else {
            $type = $_POST['type'];
        ...
        if (empty($entry_date) || $amount <= 0 || !in_array($type, ['deposit', 'withdrawal'])) {
            ...
            if ($entry_id_to_update > 0) {
                // --- UPDATE (Admin Only check inside) ---
                if (!hasAdminAccess()) {
...
<p class="text-gray-600 mb-8">
    <?php if (hasAdminAccess()): ?>
        A simple, private ledger to record personal savings and costs. This is not connected to sales data.
    <?php else: ?>
        Use this form to record any daily operating costs.
    <?php endif; ?>
</p>
...
<?php if (hasAdminAccess()): ?>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-lg">
            ...
            <div class="text-3xl font-bold text-gray-900"><?php echo e(number_format($total_deposits_range, 0)); ?> BDT
            </div>
        </div>
        <div class="bg-white p-6 rounded-2xl shadow-lg">
            <div class="text-sm font-medium text-gray-500">Costs in Range</div>
            <div class="text-3xl font-bold text-gray-900">-<?php echo e(number_format($total_withdrawals_range, 0)); ?> BDT
            </div>
        </div>
    </div>
...
<?php endif; // End Admin Only View ?>


<!-- Main Content Grid -->
<div class="grid grid-cols-1 <?php echo hasAdminAccess() ? 'lg:grid-cols-3' : 'max-w-2xl mx-auto'; ?> gap-8">

    <!-- Column 1: Add/Edit Form -->
    <div class="<?php echo hasAdminAccess() ? 'lg:col-span-1' : ''; ?>">
        <div class="bg-white p-6 rounded-2xl shadow-lg sticky top-8">
            ...
            <form action="ledger.php<?php echo ($form_mode === 'edit') ? '?action=edit&id=' . e($entry_id) : ''; ?>"
                ...
                <div>
                    ...
                    <input type="date" id="entry_date" name="entry_date" value="<?php echo e($entry_date_form); ?>"
                        required
                        class="mt-1 block w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-500">
                </div>
                ...
                <div>
                    <label for="description" class="block text-sm font-medium text-gray-700">Description
                        (Optional)</label>
                    <input type="text" id="description" name="description" value="<?php echo e($description_form); ?>"
                        placeholder="e.g., From cash drawer"
                        class="mt-1 block w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-500">
                </div>

                <div class="flex space-x-2">
                    <button type="submit" name="submit_entry"
                        class="w-full py-3 px-4 <?php echo ($form_mode === 'edit') ? 'bg-orange-600 hover:bg-orange-700' : 'bg-green-600 hover:bg-green-700'; ?> text-white font-medium rounded-lg shadow-md">
                        <?php echo ($form_mode === 'edit') ? 'Save Changes' : 'Add Cost'; ?>
                    </button>
                    <?php if ($form_mode === 'edit'): ?>
                        <a href="ledger.php"
    ...
    <?php if (hasAdminAccess()): ?>
        <!-- Column 2: Monthly & Daily View (ADMIN ONLY) -->
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <?php if (empty($monthly_summary)): ?>
                                <tr>
                                    <td colspan="4" class="px-6 py-4 text-center text-gray-500">No entries recorded yet.</td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($monthly_summary as $month):
                                    ...
                                    <tr>
                                        <td class="px-6 py-4">
                                            <div class="text-sm font-medium text-gray-900"><?php echo e($month['month_name']); ?>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 text-right">
                                            <div class="text-sm text-green-600">
                                                <?php echo number_format($month['total_deposits'], 0); ?></div>
                                        </td>
                                        <td class="px-6 py-4 text-right">
                                            <div class="text-sm text-red-600">
                                                -<?php echo number_format($month['total_withdrawals'], 0); ?></div>
                                        </td>
                                        <td class="px-6 py-4 text-right">
                                            <div
                                                class="text-sm font-medium <?php echo ($net_change >= 0) ? 'text-green-600' : 'text-red-600'; ?>">
                                                <?php echo number_format($net_change, 0); ?></div>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <?php if (empty($daily_entries)): ?>
                                <tr>
                                    <td colspan="5" class="px-6 py-4 text-center text-gray-500">No entries found in this date
                                        range.</td>
                                </tr>
                            <?php else: ?>
                                    ...
                                    <tr>
                                        <td class="px-6 py-4">
                                            <div class="text-sm text-gray-900">
                                                <?php echo e(date('M d, Y', strtotime($entry['entry_date']))); ?></div>
                                        </td>
                                        <td class="px-6 py-4">
                                            <?php if ($entry['type'] == 'deposit'): ?>
                                                <span
                                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Deposit</span>
                                            <?php else: ?>
                                                <span
                                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">Cost</span>
                                            <?php endif; ?>
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="text-sm text-gray-500"><?php echo e($entry['description']); ?></div>
                                        </td>
                                        <td class="px-6 py-4 text-right">
                                            <div
                                                class="text-sm font-medium <?php echo ($entry['type'] == 'deposit') ? 'text-green-600' : 'text-red-600'; ?>">
    ...
    <?php endif; // End Admin Only Column 2 ?>
